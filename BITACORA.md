...
| **Portal web (parte 4)** | En `main` y levantado (`make frontend`, http://localhost:8020, contraseña `FRONTEND_CLAVE` de `.env`): panel, explorador, asistente (Ollama y RAG), tiempo real, privacidad y auditoría, operaciones y documentación. El grafo de Documentación, el panel y el tiempo real se mueven con la actividad real (cargas de Airflow, simulador, Spark publicando). Demostración pública en Vercel con datos grabados (`parte4_frontend/demo`). 429 tests de Python, 17 de Scala y 165 de Vitest |
...
- **Rama / commits:** `tarea/portal-animaciones` · pendiente de commit
- **Qué he hecho:**
